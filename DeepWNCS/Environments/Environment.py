...
class environment():
    def __init__(self, num_plant, pend_conf, network = 'wireless'):
    ...
    def reset(self):
        ...
        for i in range(self.num_plant):
            plant = Pendulum(network = self.network, pend_conf = self.pend_conf['pend_%s'%(i)])
    ...
    def get_reward(self, t, done, schedule):
        '''
            <output>
            reward : sum of error of each plant
        '''
        errorVector = self.get_errorVector()
    ...
    def action_to_schedule_A2C(self, action):
        '''
        action: scalar [0, 1, ..., action_dim] -> [0, 0.5, ..., ]
        '''
        return action/2
    ...
    def get_current_env_goal(self):
        ...
        theta_ref = 0
        for plant in self.plants:
            cart_pos_refs.append(plant.currReference())
            theta_refs.append(theta_ref)
        return cart_pos_refs, theta_refs
