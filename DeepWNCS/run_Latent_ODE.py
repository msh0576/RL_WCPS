...
from BRITS_project.utils import linesplot, linesubplot
...
if __name__ == '__main__':
	...
	# print('num_batches:', num_batches)	# 160

	for itr in range(1, num_batches * (args.niters + 1)):	# around 48000
		optimizer.zero_grad()
		...
		# print("observed_data shape:", batch_dict['observed_data'].shape)	# tensor(GPU) [50, 100, 14]
		# print("observed_tp shape:", batch_dict['observed_tp'].shape)	# tensor(GPU) [100,]
		# print("data_to_predict shape:", batch_dict['data_to_predict'].shape)	# tensor(GPU) [50, 100, 14]
		# print("tp_to_predict shape:", batch_dict["tp_to_predict"].shape)	# tensor(GPU) [100,]
		# print("mask shape:", batch_dict["observed_mask"].shape)	# tensor(GPU) [50, 100, 14]
		# print("mask_predicted_data:", batch_dict['mask_predicted_data'])	# None
		# print("mode", batch_dict['mode'])	# interp
		# print("labels:", batch_dict['labels'])	# None
		...
		if itr % 300 == 0:
			# figure
			train_observed_tp = batch_dict['observed_tp'].detach().cpu().numpy()
			train_observed_data = batch_dict['observed_data'].detach().cpu().numpy()
			train_pred_y = train_res['pred_y'][0,:,:,:].cpu().numpy()
			results_dir = os.path.join('results', 'cartpole-swingup_default')
			# linesubplot(
			# 	xs = train_observed_tp,
			# 	ys_list1 = [train_observed_data[0, :, 0].tolist()],
			# 	ys_list2 = [train_pred_y[0, :, 0].tolist()],
			# 	legends1 = ['1-dim0'],
			# 	legends2 = ['2-dim0'],
			...
			linesubplot(
				xs = train_observed_tp,
				ys_list1 = [train_observed_data[0, :, 0].tolist(), train_observed_data[0, :, 1].tolist(), train_observed_data[0, :, 2].tolist(), train_observed_data[0, :, 3].tolist(), train_observed_data[0, :, 4].tolist()],
				ys_list2 = [train_pred_y[0, :, 0].tolist(), train_pred_y[0, :, 1].tolist(), train_pred_y[0, :, 2].tolist(), train_pred_y[0, :, 3].tolist(), train_pred_y[0, :, 4].tolist()],
