...
if args.experience_replay != '' and os.path.exists(args.experience_replay):
  ...
  for s in range(1, args.seed_episodes + 1):
    ...
    # print("reset obs:", observation, "shape:", observation.shape) # tensor(CPU), [1, 3, 64, 64]  # MyEnv: tensor, [1, 5]
    while not done:
      # print("time: {}s".format(round(t*0.01,3)))
      action = env.sample_random_action() # tensor, [1,]
      next_observation, reward, done = env.step(action)
      # print("next_observation shape:", next_observation.shape)  # tensor, [1, 5]
      # print("next_observation:", next_observation)  # tensor(CPU)
...
def update_belief_and_act(args, env, planner, transition_model, encoder, belief, posterior_state, action, observation, explore=False):
  ...
  # action.size() # torch.Size([1, 1]) # MyEnv: [10, 1]  
  # encoder(observation).unsqueeze(dim=0).shape  # torch.Size([1, 1, 1024])
  ...
  if explore:
    action = torch.clamp(Normal(action, args.action_noise).rsample(), -0.5, 0.5) # Add gaussian exploration noise on top of the sampled action
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes, initial=metrics['episodes'][-1] + 1):
  ...
  for s in tqdm(range(args.collect_interval)):
    ...
    # observations.shape  # torch.Size([50 (chunk), 50 (batch), 3, 64, 64]) on GPU
    # bottle(encoder, (observations[1:], )).shape  # torch.Size([49 (chunk), 50 (batch), 1024])
    ...
    imged_beliefs, imged_prior_states, imged_prior_means, imged_prior_std_devs = imagination_traj # torch.Size([14 (horizon), 2450, 200]),  torch.Size([14, 2450, 30]) 
    with FreezeParameters(model_modules + value_model.modules):
      imged_reward = bottle(reward_model, (imged_beliefs, imged_prior_states))  # torch.Size([14, 2450])
      value_pred = bottle(value_model, (imged_beliefs, imged_prior_states)) # torch.Size([14, 2450])
    returns = lambda_return(imged_reward, value_pred, bootstrap=value_pred[-1], discount=args.discount, lambda_=args.disclam) # torch.Size([14, 2450])
    ...
    with torch.no_grad():
      value_beliefs = imged_beliefs.detach()  # torch.Size([14, 2450, 200])
      value_prior_states = imged_prior_states.detach()  # torch.Size([14, 2450, 30])
      target_return = returns.detach()
    # bottle(value_model, (value_beliefs, value_prior_states)).shape  # torch.Size([14, 2450])
    ...
    '''
    value_dist.log_prob(target_return) 계산 시, 만약 value_dist이 하나의 평균과 표편을 가지면 target_return 의 사이즈가 [14, 2450]이어도 
    각 element의 log_pdf 결과의 합으로 나타난다. 즉, scalar output이다.
    만약 value_dist 의 사이즈가 [14, 2450]이면, 해당 크기만큼의 평균/분산 정보를 가지고 있고, output 크기도 [14, 2450]이 된다.
    '''
  ...
  with torch.no_grad():
    ...
    for t in pbar:
      ...
      # print("observation:", observation)  # tensor(CPU)
      # print("reward:", reward)  # scalar
  ...
  if episode % args.test_interval == 0:
    ...
    test_envs = EnvBatcher(Env, (args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth, args.num_plant), {}, args.test_episodes)
    ...
    with torch.no_grad():
      ...
      for t in pbar:
        ...
        total_rewards += reward.numpy().squeeze() # ndarray, [10, ]
