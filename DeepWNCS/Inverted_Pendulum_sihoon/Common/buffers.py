# -*- coding: utf-8 -*-
"""
Created on Sun Nov  8 15:32:51 2020
...
class ReplayBufferLSTM2:
    ...
    def sample(self, batch_size):
        ...
        min_trajectory = min(len(sample[2]) for sample in batch)
        
        for sample in batch:
            (h_in, c_in), (h_out, c_out), state, action, last_action, reward, next_state, done = sample
            s_lst.append(state[:min_trajectory]) 
            a_lst.append(action[:min_trajectory])
            la_lst.append(last_action[:min_trajectory])
            r_lst.append(reward[:min_trajectory])
            ns_lst.append(next_state[:min_trajectory])
            d_lst.append(done[:min_trajectory])
