...
from latent_ode_master.lib.likelihood_eval import *

from torch.distributions.multivariate_normal import MultivariateNormal
from torch.distributions.normal import Normal
from torch.distributions import kl_divergence, Independent
from latent_ode_master.lib.base_models import VAE_Baseline
	...
	def get_reconstruction(self, time_steps_to_predict, truth, truth_time_steps, 
		...
		if isinstance(self.encoder_z0, Encoder_z0_ODE_RNN) or \
			...
			if mask is not None:
				...
				# print("truth_w_mask shape:", truth_w_mask.shape)	# tensor [50, 100, 28]
			first_point_mu, first_point_std = self.encoder_z0(
				truth_w_mask, truth_time_steps, run_backwards = run_backwards)
			# print("first_point_mu:", first_point_mu.shape)	# tensor(GPU) [1, 50, 15]

			# here, n_traj_samples = 3
			means_z0 = first_point_mu.repeat(n_traj_samples, 1, 1)
			# print("means_z0:", means_z0.shape)	# tensor(GPU) [3, 50, 15]
			sigma_z0 = first_point_std.repeat(n_traj_samples, 1, 1)
			first_point_enc = utils.sample_standard_gaussian(means_z0, sigma_z0)
			# print("first_point_enc shape:", first_point_enc.shape)	# tensor(GPU) [3, 50, 15]
		else:
		...
		# print('--------first_point_enc_aug:', first_point_enc_aug)
		sol_y = self.diffeq_solver(first_point_enc_aug, time_steps_to_predict)	
		# encoder에 있던 z0_diffeq_solver.ode_func 과는 다름. 하나의 time step에서 dy/dt를 계산함
		if self.use_poisson_proc:
