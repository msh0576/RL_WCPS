...
from latent_ode_master.lib.utils import get_device
...
class Encoder_z0_ODE_RNN(nn.Module):
	...
	def __init__(self, latent_dim, input_dim, z0_diffeq_solver = None, 
		...
		if z0_dim is None:
			...
			self.z0_dim = z0_dim
		# print("self.z0_dim :", self.z0_dim)	# int, 15
	...
	def forward(self, data, time_steps, run_backwards = True, save_info = False):
		...
		means_z0 = last_yi.reshape(1, n_traj, self.latent_dim)	# tensor, [1, 50, 30]
		std_z0 = last_yi_std.reshape(1, n_traj, self.latent_dim)

		# print("torch.cat((means_z0, std_z0), -1) shape:", torch.cat((means_z0, std_z0), -1).shape)	# tensor, [1, 50, 60 (mean+std)]
		# print("self.transform_z0( torch.cat((means_z0, std_z0), -1)) shape:", self.transform_z0( torch.cat((means_z0, std_z0), -1)).shape)	# tensor, [1, 50, 30]
	...
	def run_odernn(self, data, time_steps, 
		...
		'''
			Input:
				data: tensor, [50 (batch), 100 (traj len), 28]
				time_steps: tensor, [100,]
			Output:
				yi: tensor, [1, 50, 30 (latent_dim)]
				yi_std: tensor, [1, 50, 30]
				latent_ys: tensor [1, 100, 50 , 30]
		'''
		# IMPORTANT: assumes that 'data' already has mask concatenated to it 
		n_traj, n_tp, n_dims = data.size()
		extra_info = []
		...
		# print("prev_y shape:", prev_y.shape)	# tensor [1, 50, 30]
		...
		for i in time_points_iter:
			if (prev_t - t_i) < minimum_step:
				...
				# print("time_points shape:", time_points.shape)	# tensor [2,]
				inc = self.z0_diffeq_solver.ode_func(prev_t, prev_y) * (t_i - prev_t)

				assert(not torch.isnan(inc).any())

				ode_sol = prev_y + inc
				# print("ode_sol shape 0:", ode_sol.shape)	# torch [1, 50, 30]
				ode_sol = torch.stack((prev_y, ode_sol), 2).to(device)
				# print("ode_sol shape 1:", ode_sol.shape)	# torch [1, 50, 2, 30]
				assert(not torch.isnan(ode_sol).any())
			else:
				n_intermediate_tp = max(2, ((prev_t - t_i) / minimum_step).int())

				time_points = utils.linspace_vector(prev_t, t_i, n_intermediate_tp)
				# print("prev_y:{}, times_points:{}".format(prev_y, time_points))
				# print('=======run_ode-rnn,,, prev_y:',prev_y)
				# print("time_points:", time_points)
				ode_sol = self.z0_diffeq_solver(prev_y, time_points)
				# print("ode_sol shape 2:", ode_sol.shape)	# torch [1, 50, 2, 30]
				assert(not torch.isnan(ode_sol).any()), "prev_y:{}".format(prev_y)
			...
			# print("xi shape:", xi.shape)	# tensor, [1, 50, 28]
		...
		# print("latent_ys shape:", latent_ys.shape)	# tensor [1, 100, 50 , 30]

		assert(not torch.isnan(yi).any())
		assert(not torch.isnan(yi_std).any())
		# print("yi shape:", yi.shape)	# tensor [1, 50, 30]
