...
from latent_ode_master.lib.base_models import Baseline
...
class ODE_RNN(Baseline):
	...
	def get_reconstruction(self, time_steps_to_predict, data, truth_time_steps, 
		...
		assert(len(truth_time_steps) == len(time_steps_to_predict))	# why??
		assert(mask is not None)
		
		data_and_mask = data
		if mask is not None:
			data_and_mask = torch.cat([data, mask],-1)
		# print("data shape:", data.shape)	# tensor [50, 100, 6]
		# print("data_and_mask:", data_and_mask.shape)	# tensor [50, 100, 12]
