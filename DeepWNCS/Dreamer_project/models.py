...
def bottle(f, x_tuple):
  '''
    observation feature를 encoder의 인풋으로 하여, [trajectory, batch, features] 형태로 return 하는 듯.
  '''
...
class TransitionModel(jit.ScriptModule):
  ...
  def forward(self, prev_state:torch.Tensor, actions:torch.Tensor, prev_belief:torch.Tensor, observations:Optional[torch.Tensor]=None, nonterminals:Optional[torch.Tensor]=None) -> List[torch.Tensor]:
    '''
    Input: init_belief, init_state:  torch.Size([50, 200]) torch.Size([50 (batch-size), 30 (state_dim)])
    ...
    # print("beliefs:", beliefs)  # CPUFloatType
    ...
    return hidden # list, [beliefs, prior_states, prior_means, prior_std_devs,// posterior_states, posterior_means, posterior_std_devs]
