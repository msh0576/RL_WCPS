import numpy as np
import torch
from Dreamer_project.env import postprocess_observation, preprocess_observation_
...
class ExperienceReplay():
  def __init__(self, size, symbolic_env, observation_size, action_size, bit_depth, device):
    '''
      <Arguemtn>
        size: experienceReplay의 총 buffer 사이즈
    '''
    self.device = device
  ...
  def _retrieve_batch(self, idxs, n, L):
    ...
    # print("self.observations shape:", self.observations.shape)  # tensor, [1000000, 5]
    # print("observations shape:", observations.shape)  # tensor, [2500, 5]
  ...
  def sample(self, n, L):
    '''
      <Argument>
        n: batch size
        L: chunk size: 하나의 batch에서 가지는 양, like trajectory
    '''
    ...
    return [torch.as_tensor(item).to(device=self.device) for item in batch] # list, [observations, actions, rewards, nonterminals]
