# -*- coding: utf-8 -*-
"""
Created on Fri Nov  6 18:28:03 2020
...
class EpisodicMemory(Memory):
    def __init__(self, capacity, max_episode_length, **kwargs):
        ...
        self.trajectory = [] # Temporal list of episode     # state 에서 action 취하고 받는 reward/next_state 의 set
    ...
    def append(self, state0, action, reward, terminal, training=True):
        ...
        if len(self.trajectory) >= self.max_episode_length: # 적어도 에피소드 한번 가량의 데이터가 축적 되어야 memory에 쌓인다.
            self.memory.append(self.trajectory)
            print("append in memory, its len:", len(self.memory))
            self.trajectory = []
