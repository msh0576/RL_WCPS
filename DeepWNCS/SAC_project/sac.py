    ...
    def select_action(self, state, evaluate=False):
        '''
            Args:
                state: 
            Returns:
                output should be: tensor(CPU), [1, action_size]
        '''
        state = torch.FloatTensor(state).to(self.device)    # tensor(CPU), [1, 5]

        if evaluate is False:
            action, _, _ = self.policy.sample_v2(state)
        else:
            _, _, action = self.policy.sample_v2(state)
        # print("action shape", action.shape) # tensor(CPU), [1, 1]
        return action
